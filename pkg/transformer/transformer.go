...
type DataTable struct {
	Headers map[portfolio.SeriesKey]struct{}
	Data    map[time.Time]map[portfolio.SeriesKey]float64
}

func Pivot(key string, labels []portfolio.SeriesKey, series []*series.TimeSeries) (*DataTable, error) {
	...
	headers := make(map[portfolio.SeriesKey]struct{}, len(labels))
	for _, l := range labels {
		headers[l] = struct{}{}
	}

	data := make(map[time.Time]map[portfolio.SeriesKey]float64, len(series[0].Times))
	...
	for i1, s := range series {
		...
		for i2, t := range s.Times {
			row := data[t]
			if row == nil {
				data[t] = map[portfolio.SeriesKey]float64{}
			}
...
func CreateWeightedSum(dt *DataTable, weights portfolio.PortfolioWeightings) (*series.TimeSeries, error) {
	if dt == nil {
	...
	acc := func(row map[portfolio.SeriesKey]float64) float64 {
		total := float64(0)
		for k, v := range row {
			total += weights[k] * v
		}
